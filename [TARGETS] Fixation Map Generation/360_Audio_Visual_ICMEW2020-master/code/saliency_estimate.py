...
def saliency(fixmap_input):
    ...
    # Normalize to 0-1 range (float)
    equi_output = equi_output/36.0
    if equi_output.max() > 0:
...
def generate_saliencymap(fixation_maps, segment_no, output_folder):
    ...
    for seg_count, fixmap in enumerate(fixation_maps):
        ...
        # FIXED: Convert float64 to uint8 before saving
        ...
        # ADDED: Zero-padding for correct sorting (e.g. salmap_f_00001.png)
        imageio.imwrite(os.path.join(salmap_folder, f'salmap_f_{seg_count:05d}.png'), saliency_map_uint8)
